# Churn-Prediction
Classify customers at risk of churn for Stanford course Machine Learning for Business with Python



#Colab link: Customer_churn_prediction_TradingPlatform_Tech68.ipynb
##1) Overview
...
##2) Data
  ...
  •  Equity_value_data: user-day level close_equity.
  •  User_features: risk_tolerance, investment_experience, liquidity_needs, platform (Android/iOS/both), instrument_type_first_traded, time_horizon, time_spent, first_deposit_amount, etc.

#Note: Given data does not have Churn labels, so I created a churn label with below definition.

##3) Churn Label Definition
  • Define a day as low-equity if close_equity < $10.
...
#Summary: 
XGBoost (AUC: 0.817) and LightGBM (AUC: 0.815) perform similarly and both exceed the Logistic Regression baseline (AUC: 0.797). At their F1‑optimized thresholds, XGBoost shows slightly fewer false positives/negatives.
##7) Conclusions
